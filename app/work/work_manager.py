...
pindex = Project(
    ...
    description = "Un site vitrine d'un développeur d'applications. Codé en python 3 avec django web framework.
...
Il existe plusieurs des façons à gérer le contenu de la page web. Django administration est très utile. L'accès à la base de donnée est réalisé dans un envirenment objet, on code les classes nous-même.
Une autre possibilté est django rest framework. C'est un api rest, un ensemble de définitions et de protocoles qui facilite la création et l'intégration de logiciels d'applications.",
    place='simplon.co, Grenoble, FRANCE',
    technology = 'python3.8, Django-3.2.4, django rest framework 3.12.4, jupiterlab 0.1.1, SQLite 3, bootstrap/4.1.3',
